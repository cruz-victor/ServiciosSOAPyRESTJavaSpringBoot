...
@Service
@WebService(targetNamespace = "https://siat.impuestos.gob.bo/FacturaElectronicaPrevalorada")
public class ServicioFacturacion implements IServicioFacturaElectronicaPrevalorada {
	...
	public IRespuesta recepcion(ISolicitudRecepcion solicitud) {
		...
		try {
			...
			vArchivoXml.setP_cuape(solicitud.getCuape());
			...
			vArchivoXml.setvOrigenServicio(Parametros.F_MODALIDAD_PREVALORADA_ELECTRONICA);
	...
	public IRespuesta anulacion(ISolicitudAnulacion solicitud) {
		...
		try {
			...
			vArchivoXml.setP_cufp(solicitud.getCufp());
	...
	@Override
	public IRespuesta validacionRecepcionPorCufp(ISolicitudValidacionRecepcionPorCufp solicitud) {
		XmlRecepcionGenericoDto vArchivoXml = new XmlRecepcionGenericoDto();

		Respuesta vRespuesta = new Respuesta();
		LOG.info("validacionRecepcionPorCufp||Iniciando");
		try {
			vArchivoXml.setP_codigo_sistema(solicitud.getCodigoSistema());
			vArchivoXml.setP_tipo_ambiente_id(solicitud.getCodigoAmbiente());
			vArchivoXml.setP_tipo_emision_id(solicitud.getCodigoEmision());
			...
			LogExcepcion.registrar(e, LOG, MethodSign.build(vRespuesta));
			LOG.info("validacionRecepcionPorCufp||Excepcion");
			vRespuesta.setCodigoRecepcion(0L);
			vRespuesta.setTransaccion(false);
			vRespuesta.setCodigoEstado(CodigosMensajesServiciosSOAPServiceImpl.RECEPCION_RECHAZADA);
			vRespuesta.getListaCodigosRespuestas()
		...
		LOG.info("validacionRecepcionPorCufp||Finalizado");
		return vRespuesta;
	}
