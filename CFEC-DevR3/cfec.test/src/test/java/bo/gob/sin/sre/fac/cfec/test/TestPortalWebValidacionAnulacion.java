...
@Transactional
@Rollback
public class TestPortalWebValidacionAnulacion {
	private static final Logger LOG = LoggerFactory.getLogger(TestPortalWebValidacionAnulacion.class);
	...
	@Rollback(false)
	@Test
	public void validacionAnulacionFacturaPortalWebP01() throws Exception {
		RespuestaJsonDto vRespuesta = new RespuestaJsonDto();
		try {

			LOG.info("validacionAnulacionFacturaPortalWebP01 ==>" + vRespuesta.toString());
			...
			vXmlRecGenDto.setP_recepcion_anulacion_id(solicitud.getCodigoRecepcion());

			vRespuesta = iServiciosClientRest.validacionAnulacionFacturaPortalWeb(vXmlRecGenDto);
